...
export default withErrorHandling(async function handler(req, res) {
  ...
  // Announced here, once, rather than at the end of each handler above.
  // Every non-GET route on this endpoint changes something a polling plugin
  // acts on — the bingo_active switch most of all — and the poll is cached
  // until told otherwise (see _lib/board-cache.ts). Doing it at the dispatcher
  // means a route added later cannot forget to, which is the exact failure
  // db/schema.sql avoids for board_changed_at by using triggers rather than a
  // bump() call at every write site.
  if (req.method !== "GET") {
